...
class TestServerQueries:
    ...
    async def setup_class(cls) -> None:
        ...
        try:
            await connection.execute(f'DROP DATABASE IF EXISTS {cls.settings.postgres_test_db}')
    ...
    async def test_status(self, test_data, connection: asyncpg.Connection) -> None:
        ...
        """
        not_existing_client_status = await query_status(connection, '00000000-0000-0000-0000-000000000000')
        assert not_existing_client_status is None

        client_status = await query_status(connection, '26c940a1-7228-4ea2-a3bc-e6460b172040')
    ...
    async def test_add(self, test_data, connection: asyncpg.Connection) -> None:
        ...
        client_status = await query_add(connection, '26c940a1-7228-4ea2-a3bc-e6460b172040', -1000)
        assert client_status['balance'] == 1700
        assert client_status['hold'] == 300
    ...
    async def test_subtract(self, test_data, connection: asyncpg.Connection) -> None:
        ...
        client_status = await query_subtract(connection, '26c940a1-7228-4ea2-a3bc-e6460b172040', -1000)
        assert client_status['balance'] == 1700
        assert client_status['hold'] == 300
        ...
        with pytest.raises(NotEnoughMoneyError):
            await query_subtract(connection, '26c940a1-7228-4ea2-a3bc-e6460b172040', 2000)
